...
SIGNALWIRE_FROM = os.environ.get("SIGNALWIRE_FROM_NUMBER", "")
SIGNALWIRE_SPACE = os.environ.get("SIGNALWIRE_SPACE_URL", "")  # e.g. "eg23.signalwire.com"
...
def _signalwire_messages_url() -> str:
    return f"https://{SIGNALWIRE_SPACE}/api/laml/2010-04-01/Accounts/{SIGNALWIRE_PROJECT_ID}/Messages.json"


def _signalwire_auth() -> HTTPBasicAuth:
    return HTTPBasicAuth(SIGNALWIRE_PROJECT_ID, SIGNALWIRE_API_TOKEN)
...
def send_sms(to: str, message: str) -> dict:
    """Send an SMS via SignalWire's LaML REST API (native HTTP, no SDK)."""
    response = requests.post(
        _signalwire_messages_url(),
        auth=_signalwire_auth(),
        data={
    ...
    if response.status_code >= 400:
        raise RuntimeError(f"SignalWire send failed [{response.status_code}]: {response.text}")
